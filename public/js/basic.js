...
function vote_helper(action, identifier, note) {
    ...
    $.ajax({
        ...
        url: '/vote/' + action,
        data: {
            'timestamp': Math.floor((date ? Date.parse(date) : Date.now()) / 1000),
        ...
        success: function (result) {
            ...
            intervalVotes = setInterval(function () {
                vote_get()
            }, Math.floor(5000 * voting_over_interval_multiplier));
        ...
        error: function(xhr, status, error) {
            ...
            if (result && typeof result.alert != 'undefined') {
                alert(result.alert);
            } else {
...
function vote_up(identifier) {
    vote_helper('up', identifier, null, 0);
...
function vote_down(identifier) {
    vote_helper('down', identifier, null, 0);
    $('#dialog_vote_summary').show();
    adapt_button_vote_summary_toggle();
}
...
function vote_special(identifier) {
    ...
    vote_helper('special', identifier, null, 0);
    $('#dialog_vote_summary').show();
    adapt_button_vote_summary_toggle();
}
...
function vote_set_note(note) {
    vote_helper('set_note', null, note, 0);
    $('#dialog_vote_summary').show();
    adapt_button_vote_summary_toggle();
}
...
// get votes
function vote_get() {
    vote_helper('all', null, null, 0);
}

// delete vote
function vote_delete() {
    $('#noteInput').val('');
    vote_helper('delete', null, null, 0);
}
...
function vote_delete_part(identifier) {
    ...
    vote_helper('delete_part', identifier, null, 0);
}
